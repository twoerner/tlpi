...
static void print_proc_and_children (unsigned idx, unsigned level);
...
main (void)
{
	...
	memset (procInfo_pG, 0, procInfoCnt_G * sizeof (*procInfo_pG));
...
static void
print_proc_and_children (unsigned idx, unsigned level)
{
	unsigned i;
	...
	if (procInfo_pG[idx].valid && !procInfo_pG[idx].printed) {
		...
		for (i=0; i<procInfoCnt_G; ++i)
			if (procInfo_pG[i].ppid == procInfo_pG[idx].pid)
				if (procInfo_pG[i].valid && !procInfo_pG[i].printed)
					print_proc_and_children (i, level+1);
	}
}
